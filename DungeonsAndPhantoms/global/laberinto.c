...
void leerConfiguracion(tConfiguracion *config) {
    ...
    if (archivo == NULL) {
        ...
        config->obstaculosEliminados = 0.6;
    ...
    while (fgets(linea, sizeof(linea), archivo)) {
        ...
        } else if(strstr(linea, "porcentaje_obstaculos:")){
            sscanf(linea, "porcentaje_obstaculos: %f", &config->obstaculosEliminados);
        }
    }
...
int generarLaberinto(tLaberinto *lab, tConfiguracion config) {
    int i, j, borde, eliminadas, paredesAEliminar,bordeOpuesto, x, y, startX, startY, paredesInternas;
    srand(time(NULL));
